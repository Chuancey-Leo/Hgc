...
<!--手动加在语言，避免在ie下有时因为加载语言失败导致编辑器加载失败-->
...
<script type="text/javascript">
	...
	function submitData(){
		...
		if(title==null || title==''){
			...
			$.post("${pageContext.request.contextPath}/user/admin/saveNews",{'id':'${param.id}','title':title,'newsTypeId':newsTypeId,'content':content,'keyWords':keyWord},function(result){
				if(result.success){
					alert("新闻修改成功！");
				}else{
					alert("新闻修改失败！");
...
<div id="p" class="easyui-panel" title="修改博客" style="padding: 10px">
 	<table cellspacing="20px">
   		...
   		<tr>
   			<td>所属类别：</td>
   			<td>
   				<select class="easyui-combobox" style="width: 154px" id="newsTypeId" name="newsType.id" editable="false" panelHeight="auto" >
        ...
        UE.ajax.request("${pageContext.request.contextPath}/user/admin/findById",
            {
                ...
                onsuccess:function(result){
                	...
                	$("#keyWord").val(result.keyWord);
       				$("#newsTypeId").combobox("setValue",result.newsTypeId);
