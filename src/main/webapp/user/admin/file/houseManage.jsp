				...
				<div class="bs-example bs-example-tabs">
					<ul id="myTab" class="nav nav-tabs" role="tablist">
						...
						<li role="presentation" class="search-li">
							<form action="${pageContext.request.contextPath}/user/admin/searchFile" method="get" class="navbar-form navbar-left form-inline" style="margin-bottom: -7px;" role="search">
								<div class="form-group">
									<input maxlength="100" style="margin-top:-6px;width:auto; display:inline-block;margin-left:10px;" type="text" required="required" name="content" class="form-control" placeholder="输入要搜索的内容..">
					...
					<div role="tabpanel" class="tab-pane fade" id="colNumSet" aria-labelledby="colNumSet-tab">
						...
						<table class="table" fitColumns="true" pagination="true" rownumbers="true" fit="true">
							...
							<c:forEach var="house" items="${houseList}">
								<tr>
									<td><c:out value="${house.title}" escapeXml="true"/></td>
									<td><fmt:formatDate value="${house.releaseDate }" type="date" pattern="yyyy-MM-dd HH:mm"/></td>
									<td><button type="button" class="btn btn-default" onclick="deleteData(${house.id})">删除</button></td>
									<td><button type="button" class="btn btn-default"
												onclick="window.location.href='${pageContext.request.contextPath}/user/admin/updateFile/${house.id}'">更改</button></td>
									<td><button type="button" class="btn btn-default"
												onclick="window.location.href='${pageContext.request.contextPath}/file/preview/${house.id}'">预览</button></td>
