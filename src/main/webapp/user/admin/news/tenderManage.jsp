		...
		<div class="main-right col-sm-12 col-xs-12 col-md-8 col-lg-9">
			...
			<div class="bs-example bs-example-tabs">
				<ul id="myTab" class="nav nav-tabs" role="tablist">
					...
					<li role="presentation" class="">
						<a href="#colNumSet" role="tab" id="colNumSet-tab" data-toggle="tab" aria-controls="colNumSet" aria-expanded="false">招标信息</a>
					</li>
				...
				<h4>总数：${sum}</h4>
				<div role="tabpanel" class="tab-pane fade" id="colNumSet" aria-labelledby="strolPicSet-tab">
					<!--下面是设置服务动态-->
					<table class="table" fitColumns="true" pagination="true" rownumbers="true"
						...
						<c:forEach var="tender" items="${tenderList}">
							<tr>
								<td><c:out value="${tender.title}" escapeXml="true"/></td>
								<td><c:out value="${tender.author}" escapeXml="true"/></td>
								<td><fmt:formatDate value="${tender.releaseDate }" type="date" pattern="yyyy-MM-dd HH:mm"/></td>
								<td><button type="button" class="btn btn-default" onclick="deleteData(${tender.id})">删除</button></td>
								<td><button type="button" class="btn btn-default"
											onclick="window.location.href='${pageContext.request.contextPath}/user/admin/updateNews/${tender.id}'">更改</button></td>
								<td><button type="button" class="btn btn-default" onclick="toTop(${tender.id})">置顶</button></td>
								<td><button type="button" class="btn btn-default" onclick="cancelTop(${tender.id})">取消置顶</button></td>
								<td><button type="button" class="btn btn-default" onclick="window.location.href='${pageContext.request.contextPath}/news/${tender.id}'">预览</button></td>
