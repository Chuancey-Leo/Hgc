		...
		<div class="main-right col-sm-12 col-xs-12 col-md-8 col-lg-9">
			...
			<div class="bs-example bs-example-tabs">
				<ul id="myTab" class="nav nav-tabs" role="tablist">
					...
					<li role="presentation" class="">
						<a href="#strolPicSet" id="strolPicSet-tab" role="tab" data-toggle="tab" aria-controls="strolPicSet" aria-expanded="false">服务指南</a>
					</li>
					<li role="presentation" class="">
					...
					<li role="presentation" class="search-li">
						<form action="${pageContext.request.contextPath}/user/admin/search" method="get" class="navbar-form navbar-left" style="margin-bottom: -7px;" role="search">
							<div class="form-group">
								<input maxlength="100" type="text" style="margin-top: -6px" required="required" name="content" class="form-control manage-search" placeholder="输入要搜索的内容..">
								<input class="btn btn-primary" style="margin-top: -6px" type="submit" value="搜索">
							</div>
						</form>
					</li>
				</ul>
				...
				<div role="tabpanel" class="tab-pane fade" id="strolPicSet" aria-labelledby="strolPicSet-tab">
					...
					<table class="table" fitColumns="true" pagination="true" rownumbers="true"
						   url="${pageContext.request.contextPath}/user/admin/service" fit="true">
						<thead>
						<tr>
						...
						<c:forEach var="service" items="${serviceList}">
							<tr>
								<td><c:out value="${service.title}" escapeXml="true"/></td>
								<td><c:out value="${service.author}" escapeXml="true"/></td>
								<td><fmt:formatDate value="${service.releaseDate }" type="date" pattern="yyyy-MM-dd HH:mm"/></td>
								<td><button type="button" class="btn btn-default" onclick="deleteData(${service.id})">删除</button></td>
								<td><button type="button" class="btn btn-default"
											onclick="window.location.href='${pageContext.request.contextPath}/user/admin/updateNews/${service.id}'">更改</button></td>
								<td><button type="button" class="btn btn-default" onclick="toTop(${service.id})">置顶</button></td>
								<td><button type="button" class="btn btn-default" onclick="cancelTop(${service.id})">取消置顶</button></td>
								<td><button type="button" class="btn btn-default" onclick="window.location.href='${pageContext.request.contextPath}/news/${service.id}'">预览</button></td>
